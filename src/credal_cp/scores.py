...
# APS score for classification
class APSScore(Scores):
    """
    Conformity score for regression (residuals)
    --------------------------------------------------------
    Input: (i)    base_model: Point prediction model object with fit and predict methods.
    ...
    def fit(self, X, y):
        """
        Fit the classification base model to training data
        --------------------------------------------------------
        Input: (i)    X: Training feature matrix.
               (ii)   y: Training label vector.
    ...
    def compute(
        ...
        Input: (i)    X_calib: Calibration feature matrix
               (ii)   y_calib: Calibration label vector
               (iii)  n_class: Number of classes
        Output: Conformity score vector
        """
        n = X_calib.shape[0]
        # predicting each probability
        cal_probs = self.base_model.predict_proba(X_calib)
    ...
    def predict(self, X_test, cutoff, ensemble=False):
        ...
        Output: Prediction intervals for test sample.
        """
        pred_probs = self.base_model.predict_proba(X_test)

        # reshaping cutoff if they are different
        if np.unique(cutoff).shape[0] > 1:
            cutoff = np.repeat(
                cutoff,
                pred_probs.shape[1],
            ).reshape(X_test.shape[0], pred_probs.shape[1])
