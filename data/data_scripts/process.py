...
def process(dataset, n_samples=None, seed=125):
    ...
    if dataset == "homes":
        df = pd.read_csv(f"data/raw/homes/kc_house_data.csv")
    ...
    if dataset == "airfoil":
        df = pd.read_table("data/raw/airfoil/airfoil_self_noise.dat", header=None)
    ...
    if dataset == "cycle":
        with zipfile.ZipFile("data/raw/cycle/CCPP.zip", "r") as file:
    ...
    if dataset == "winered":
        df = pd.read_csv("data/raw/winered/winequality-red.csv", delimiter=";")
        X, y = df.iloc[:, :11], df.iloc[:, 11]
        data = pd.DataFrame(X)
        data["target"] = y

    if dataset == "winewhite":
        df = pd.read_csv("data/raw/winewhite/winequality-white.csv", delimiter=";")
        X, y = df.iloc[:, :11], df.iloc[:, 11]
        data = pd.DataFrame(X)
        data["target"] = y

    if dataset == "bike":
        df = pd.read_csv("data/raw/bike/bike_train.csv")

        # seperating season as per values.
    ...
    if dataset == "meps19":
        df = pd.read_csv("data/raw/meps/meps_19_reg.csv")
